    ...
    void OnDrawGizmosSelected()
{
    ...
    // Ground check ray
    Vector2 groundRayStart = origin + dir * 0.2f;
    Vector2 groundRayEnd = groundRayStart + Vector2.down * groundCheckDistance;
    Debug.DrawLine(groundRayStart, groundRayEnd, Color.green);

    // Wall check ray
    Vector2 wallRayEnd = origin + dir * wallCheckDistance;
    Debug.DrawLine(origin, wallRayEnd, Color.red);

    // Optional: Show patrol range
    Gizmos.color = Color.blue;
    Gizmos.DrawLine(new Vector2(startPos.x - patrolRange, transform.position.y), new Vector2(startPos.x + patrolRange, transform.position.y));
}
